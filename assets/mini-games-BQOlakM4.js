...
import{a as e,c as t,o as n,s as r}from"./index-BiXm4Eu6.js";var i=null,a=null,o=null,s=null,c=[],l=0,u=0;function d(){return document.querySelector(`#app`)}function f(t){return i=t,a=n[t.id]||{color:`#7C3AED`,bg:`#EDE7FB`},o=e(t.id),m()}function p(){h()}function m(){if(!o)return`<div class="ex-page"><p style="padding:24px">Мини-игры для этой главы скоро появятся.</p></div>`;let e=a;return`
    ...
    </div>`}function h(){document.getElementById(`mg-back-hub`)?.addEventListener(`click`,()=>{t(()=>import(`./index-BiXm4Eu6.js`).then(e=>e.t).then(e=>{d().innerHTML=e.renderExercisesPage(i,[],[]),e.bindExercisesPage()}),__vite__mapDeps([0,1]))}),document.querySelectorAll(`.mg-mode-card`).forEach(e=>{e.addEventListener(`click`,()=>g(e.dataset.game))})}function g(e){s=e,c=P([...o[e]]),l=0,u=0,r.miniGameStart(i.id,e),y()}function _(){d().innerHTML=m(),h()}function v(){if(l++,l>=c.length){N();return}y()}function y(){let e=c[l],t=`${l+1} / ${c.length}`,n=Math.round(l/c.length*100),r=``;s===`wordOrder`&&(r=b(e)),s===`registerPick`&&(r=S(e)),s===`suffixChoice`&&(r=w(e)),s===`findError`&&(r=E(e)),s===`dialogueGap`&&(r=O(e)),s===`quickNumbers`&&(r=A(e)),d().innerHTML=`
    <div class="ex-page">
      <header class="header">
    ...
    <div class="ex-card mg-card">
      ...
      <div class="mg-feedback" id="mg-feedback"></div>
      <button class="mg-check-btn" id="mg-check" style="background:${a.color}">Проверить</button>
    </div>`}function j(e){let t=document.getElementById(`mg-input`),n=document.getElementById(`mg-check`);t?.focus();function r(){let r=t.value.trim().toLowerCase(),i=(e.acceptVariants?e.acceptVariants.map(e=>e.toLowerCase()):[e.answer.toLowerCase()]).includes(r),a=document.getElementById(`mg-feedback`);i?(u++,a.innerHTML=`<span class="mg-correct">✓ Правильно!</span>`):a.innerHTML=`<span class="mg-wrong">✗ Правильно: <em>${e.answer}</em></span>`,t.disabled=!0,n.textContent=l===c.length-1?`Завершить →`:`Дальше →`,n.onclick=v}t?.addEventListener(`keydown`,e=>{e.key===`Enter`&&r()}),n?.addEventListener(`click`,r)}function M(e){s===`wordOrder`&&x(e),s===`registerPick`&&C(e),s===`suffixChoice`&&T(e),s===`findError`&&D(e),s===`dialogueGap`&&k(e),s===`quickNumbers`&&j(e)}function N(){let e=c.length,t=Math.round(u/e*100),n=t>=80?`🎉`:t>=50?`👍`:`💪`;r.miniGameComplete(i.id,s,u,e),d().innerHTML=`
    <div class="ex-page">
      <header class="header">
