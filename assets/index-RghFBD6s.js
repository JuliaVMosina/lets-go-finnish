...
`,`"`,s.BYTE_ORDER_MARK],s.WORKERS_SUPPORTED=!r&&!!t.Worker,s.NODE_STREAM_INPUT=1,s.LocalChunkSize=10485760,s.RemoteChunkSize=5242880,s.DefaultDelimiter=`,`,s.Parser=h,s.ParserHandle=p,s.NetworkStreamer=l,s.FileStreamer=u,s.StringStreamer=d,s.ReadableStreamStreamer=f,t.jQuery&&((n=t.jQuery).fn.parse=function(e){var r=e.config||{},i=[];return this.each(function(e){if(!(n(this).prop(`tagName`).toUpperCase()===`INPUT`&&n(this).attr(`type`).toLowerCase()===`file`&&t.FileReader)||!this.files||this.files.length===0)return!0;for(var a=0;a<this.files.length;a++)i.push({file:this.files[a],inputElem:this,instanceConfig:n.extend({},r)})}),a(),this;function a(){if(i.length===0)x(e.complete)&&e.complete();else{var t,r,a,c,l=i[0];if(x(e.before)){var u=e.before(l.file,l.inputElem);if(typeof u==`object`){if(u.action===`abort`)return t=`AbortError`,r=l.file,a=l.inputElem,c=u.reason,void(x(e.error)&&e.error({name:t},r,a,c));if(u.action===`skip`)return void o();typeof u.config==`object`&&(l.instanceConfig=n.extend(l.instanceConfig,u.config))}else if(u===`skip`)return void o()}var d=l.instanceConfig.complete;l.instanceConfig.complete=function(e){x(d)&&d(e,l.file,l.inputElem),o()},s.parse(l.file,l.instanceConfig)}}function o(){i.splice(0,1),a()}}),i&&(t.onmessage=function(e){e=e.data,s.WORKER_ID===void 0&&e&&(s.WORKER_ID=e.workerId),typeof e.input==`string`?t.postMessage({workerId:s.WORKER_ID,results:s.parse(e.input,e.config),finished:!0}):(t.File&&e.input instanceof File||e.input instanceof Object)&&(e=s.parse(e.input,e.config))&&t.postMessage({workerId:s.WORKER_ID,results:e,finished:!0})}),(l.prototype=Object.create(c.prototype)).constructor=l,(u.prototype=Object.create(c.prototype)).constructor=u,(d.prototype=Object.create(d.prototype)).constructor=d,(f.prototype=Object.create(c.prototype)).constructor=f,s})}))(),1),S=`2PACX-1vQwqkX44b-HPNysrWI2Qy5AtbUv9DVW8JtDXiFBvoe7lkIbkp-TNP2VjLyli2AUzFFCHZH0HYvcfytH`,C={chapters:`0`,vocabulary:`195278083`,exercises:`1198814770`};function w(e){return`https://docs.google.com/spreadsheets/d/e/${S}/pub?gid=${e}&single=true&output=csv`}async function T(e){let t=await(await fetch(w(e))).text();return x.default.parse(t,{header:!0,skipEmptyLines:!0}).data}async function E(){let[e,t,n]=await Promise.all([T(C.chapters),T(C.vocabulary),T(C.exercises)]);return{chapters:e,vocabulary:t,exercises:n}}function D(){let e=(location.hash.replace(`#`,``)||`/`).split(`/`).filter(Boolean);return e.length===0?{page:`dashboard`}:e[0]===`chapter`&&e[1]?{page:e[2]||`chapter`,chapterId:e[1],section:e[2]||null}:e[0]===`profile`&&e[1]?{page:`profile`,profileId:e[1]}:{page:`dashboard`}}function O(e){location.hash=e}function k(e){window.addEventListener(`hashchange`,e)}var A={ch1:{color:`#7C3AED`,bg:`#EDE7FB`,img:`ch1.png`},ch2:{color:`#EC4899`,bg:`#FAE9F2`,img:`ch2.png`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`,img:`ch3.png`},ch4:{color:`#10B981`,bg:`#E8F7EE`,img:`ch4.png`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`,img:`ch5.png`}};function j(e,t,n,r){let i=e[n]?.[t]||{},a=Object.values(i).filter(e=>e.completed||e.seen).length;return r>0?Math.round(a/r*100):0}function M(e,t,n,r){let i=g(),a=A[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`,img:`ch1.png`},o=j(i,e.id,`vocabulary`,t.length),s=j(i,e.id,`exercises`,n.length);return`
  ...
  `}function N(e){document.getElementById(`btn-back`)?.addEventListener(`click`,()=>{O(`/`)}),document.querySelectorAll(`.section-btn`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.section;if(n===`grammar`){alert(`Грамматика — скоро!`);return}O(`/chapter/${e}/${n}`)})})}var P={ch1:{color:`#7C3AED`,bg:`#EDE7FB`},ch2:{color:`#EC4899`,bg:`#FAE9F2`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`},ch4:{color:`#10B981`,bg:`#E8F7EE`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`}},F=0,I=[],L=``,R=!1;function z(e,t){L=e.id,I=t,F=0,R=!1;let n=P[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`};return`
    <div class="vocab-page">
      <header class="header">
        <button class="btn-back" id="btn-back-vocab">← ${e.title_fi}</button>
        <span class="vocab-counter" id="vocab-counter">1 / ${I.length}</span>
      </header>
      ...
      <div class="vocab-container">
        <div class="flashcard" id="flashcard" style="--card-color: ${n.color}; --card-bg: ${n.bg}">
          <div class="flashcard-inner" id="flashcard-inner">
            <div class="flashcard-front">
              ${I[0]?.tag===`puhekieli`?`<span class="puhekieli-badge">🗣️ puhekieli</span>`:``}
              <span class="card-lang">финский</span>
              <div class="card-word" id="card-word-front">${I[0]?.finnish||``}</div>
              <span class="card-hint">нажми чтобы перевернуть</span>
            </div>
            <div class="flashcard-back">
              ${I[0]?.tag===`puhekieli`?`<span class="puhekieli-badge">🗣️ puhekieli</span>`:``}
              <span class="card-lang">перевод</span>
              <div class="card-word" id="card-word-back">${I[0]?.russian||``}</div>
              ${I[0]?.english?`<div class="card-word-en">${I[0].english}</div>`:``}
            </div>
          </div>
        </div>
        ...
        <div class="vocab-dots" id="vocab-dots">
          ${I.map((e,t)=>`<span class="vocab-dot ${t===0?`active`:``}" data-i="${t}"></span>`).join(``)}
        </div>
      </div>
    </div>
  `}function ee(e){let t=P[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`};L=e.id,document.getElementById(`btn-back-vocab`)?.addEventListener(`click`,()=>{O(`/chapter/${e.id}`)});let n=document.getElementById(`flashcard-inner`);document.getElementById(`flashcard`)?.addEventListener(`click`,()=>{R||(R=!0,n.classList.add(`flipped`),document.getElementById(`vocab-actions`).style.display=`flex`)});function r(r){if(y(p().activeProfile,L,F,r),F++,F>=I.length){te(e,t);return}R=!1,n.classList.remove(`flipped`),document.getElementById(`vocab-actions`).style.display=`none`;let i=I[F];document.getElementById(`card-word-front`).textContent=i.finnish,document.getElementById(`card-word-back`).textContent=i.russian,document.querySelectorAll(`.flashcard-front, .flashcard-back`).forEach(e=>{let t=e.querySelector(`.puhekieli-badge`);if(i.tag===`puhekieli`){if(!t){let t=document.createElement(`span`);t.className=`puhekieli-badge`,t.textContent=`🗣️ puhekieli`,e.prepend(t)}}else t?.remove()}),document.getElementById(`vocab-counter`).textContent=`${F+1} / ${I.length}`,document.querySelectorAll(`.vocab-dot`).forEach((e,t)=>{e.classList.toggle(`active`,t===F),t<F&&e.classList.add(`done`)})}document.getElementById(`btn-hard`)?.addEventListener(`click`,()=>r(!1)),document.getElementById(`btn-ok`)?.addEventListener(`click`,()=>r(!1)),document.getElementById(`btn-know`)?.addEventListener(`click`,()=>r(!0))}function te(e,t){document.querySelector(`.vocab-container`).innerHTML=`
    <div class="complete-screen">
      <div class="complete-emoji">🎉</div>
      <h2>Словарь главы пройден!</h2>
      <p style="color: #6B7280; margin-top: 8px">${I.length} слов изучено</p>
      <button class="btn-complete" style="background: ${t.color}" id="btn-to-chapter">
        Вернуться к главе →
      </button>
    </div>
  `,document.getElementById(`btn-to-chapter`)?.addEventListener(`click`,()=>{O(`/chapter/${e.id}`)})}var B={ch1:{color:`#7C3AED`,bg:`#EDE7FB`},ch2:{color:`#EC4899`,bg:`#FAE9F2`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`},ch4:{color:`#10B981`,bg:`#E8F7EE`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`}},ne={multiple_choice:`☑️ Выбери правильный`,fill_blank:`✏️ Заполни пропуск`,self_check:`👁️ Самопроверка`,matching:`🔗 Соедини пары`,vowel_harmony:`🔤 Гармония гласных`},V=[],H=0,U=null,W=null,G=!1;function re(e,t){return U=e,V=t,H=0,G=!1,W=B[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`},V.length?`
    <div class="ex-page">
      <header class="header">
        <button class="btn-back" id="btn-back-ex">← ${e.title_fi}</button>
        <span class="ex-counter" id="ex-counter">${H+1} / ${V.length}</span>
      </header>
      <div class="ex-progress-bar">
        <div class="ex-progress-fill" id="ex-progress-fill"
             style="width: ${K(0)}%; background: ${W.color}"></div>
      </div>
  ...
  `:`
      ...
      </div>`}function K(e){return Math.round(e/V.length*100)}function ie(e){return e.map(e=>({v:e,sort:Math.random()})).sort((e,t)=>e.sort-t.sort).map(({v:e})=>e)}function q(e,t){let n=e.type||`multiple_choice`,r=ne[n]||n,i=[e.option_a,e.option_b,e.option_c,e.option_d].filter(Boolean),a=`abcd`.indexOf((e.correct||`a`).toLowerCase()),o=i[a]||i[0],s=n===`self_check`?i:ie(i.map((e,t)=>({text:e,isCorrect:t===a})));return n===`self_check`?`
      <div class="ex-card" data-type="${n}">
        <div class="ex-type-badge" style="background: ${W.bg}; color: ${W.color}">${r}</div>
        ...
        <div class="ex-answer-reveal" id="answer-reveal" style="display:none">
          ...
          <div class="self-check-btns">
            ...
            <button class="self-btn self-right" data-result="right"
                    style="background: ${W.color}; color: white">✓ Верно</button>
          </div>
        </div>
      </div>
    `:`
  ...
  `}function ae(e){U=e,W=B[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`},document.getElementById(`btn-back-ex`)?.addEventListener(`click`,()=>{O(`/chapter/${e.id}`)}),J()}function J(){let e=V[H];e&&(G=!1,document.getElementById(`btn-show-answer`)?.addEventListener(`click`,()=>{document.getElementById(`answer-reveal`).style.display=`block`,document.getElementById(`btn-show-answer`).style.display=`none`}),document.querySelectorAll(`.self-btn`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.result,v(p().activeProfile,U.id,H,e.dataset.result),Y()})}),document.querySelectorAll(`.ex-option`).forEach(t=>{t.addEventListener(`click`,()=>{if(G)return;G=!0;let n=t.dataset.correct===`true`,r=document.querySelector(`.ex-card`).dataset.correct;document.querySelectorAll(`.ex-option`).forEach(e=>{e.dataset.correct===`true`?e.classList.add(`correct`):e===t&&!n&&e.classList.add(`wrong`),e.disabled=!0});let i=document.getElementById(`ex-feedback`);i.style.display=`block`,i.innerHTML=n?`<div class="feedback-correct" style="border-color: ${W.color}">
    ...
    <div class="complete-screen">
      ...
      <p style="color: #6B7280; margin-top: 8px">${V.length} заданий выполнено</p>
      <button class="btn-complete" style="background: ${W.color}" id="btn-to-chapter-ex">
        Вернуться к главе →
      </button>
    </div>
