...
`,`"`,s.BYTE_ORDER_MARK],s.WORKERS_SUPPORTED=!r&&!!t.Worker,s.NODE_STREAM_INPUT=1,s.LocalChunkSize=10485760,s.RemoteChunkSize=5242880,s.DefaultDelimiter=`,`,s.Parser=h,s.ParserHandle=p,s.NetworkStreamer=l,s.FileStreamer=u,s.StringStreamer=d,s.ReadableStreamStreamer=f,t.jQuery&&((n=t.jQuery).fn.parse=function(e){var r=e.config||{},i=[];return this.each(function(e){if(!(n(this).prop(`tagName`).toUpperCase()===`INPUT`&&n(this).attr(`type`).toLowerCase()===`file`&&t.FileReader)||!this.files||this.files.length===0)return!0;for(var a=0;a<this.files.length;a++)i.push({file:this.files[a],inputElem:this,instanceConfig:n.extend({},r)})}),a(),this;function a(){if(i.length===0)x(e.complete)&&e.complete();else{var t,r,a,c,l=i[0];if(x(e.before)){var u=e.before(l.file,l.inputElem);if(typeof u==`object`){if(u.action===`abort`)return t=`AbortError`,r=l.file,a=l.inputElem,c=u.reason,void(x(e.error)&&e.error({name:t},r,a,c));if(u.action===`skip`)return void o();typeof u.config==`object`&&(l.instanceConfig=n.extend(l.instanceConfig,u.config))}else if(u===`skip`)return void o()}var d=l.instanceConfig.complete;l.instanceConfig.complete=function(e){x(d)&&d(e,l.file,l.inputElem),o()},s.parse(l.file,l.instanceConfig)}}function o(){i.splice(0,1),a()}}),i&&(t.onmessage=function(e){e=e.data,s.WORKER_ID===void 0&&e&&(s.WORKER_ID=e.workerId),typeof e.input==`string`?t.postMessage({workerId:s.WORKER_ID,results:s.parse(e.input,e.config),finished:!0}):(t.File&&e.input instanceof File||e.input instanceof Object)&&(e=s.parse(e.input,e.config))&&t.postMessage({workerId:s.WORKER_ID,results:e,finished:!0})}),(l.prototype=Object.create(c.prototype)).constructor=l,(u.prototype=Object.create(c.prototype)).constructor=u,(d.prototype=Object.create(d.prototype)).constructor=d,(f.prototype=Object.create(c.prototype)).constructor=f,s})}))(),1),x=`2PACX-1vQwqkX44b-HPNysrWI2Qy5AtbUv9DVW8JtDXiFBvoe7lkIbkp-TNP2VjLyli2AUzFFCHZH0HYvcfytH`,S={chapters:`0`,vocabulary:`195278083`,exercises:`1198814770`};function C(e){return`https://docs.google.com/spreadsheets/d/e/${x}/pub?gid=${e}&single=true&output=csv`}async function w(e){let t=await(await fetch(C(e))).text();return b.default.parse(t,{header:!0,skipEmptyLines:!0}).data}async function T(){let[e,t,n]=await Promise.all([w(S.chapters),w(S.vocabulary),w(S.exercises)]);return{chapters:e,vocabulary:t,exercises:n}}function E(){let e=(location.hash.replace(`#`,``)||`/`).split(`/`).filter(Boolean);return e.length===0?{page:`dashboard`}:e[0]===`chapter`&&e[1]?{page:e[2]||`chapter`,chapterId:e[1],section:e[2]||null}:e[0]===`profile`&&e[1]?{page:`profile`,profileId:e[1]}:{page:`dashboard`}}function D(e){location.hash=e}function O(e){window.addEventListener(`hashchange`,e)}var k={ch1:{color:`#7C3AED`,bg:`#EDE7FB`,img:`ch1.png`},ch2:{color:`#EC4899`,bg:`#FAE9F2`,img:`ch2.png`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`,img:`ch3.png`},ch4:{color:`#10B981`,bg:`#E8F7EE`,img:`ch4.png`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`,img:`ch5.png`}};function A(e,t,n,r){let i=e[n]?.[t]||{},a=Object.values(i).filter(e=>e.completed||e.seen).length;return r>0?Math.round(a/r*100):0}function j(e,t,n,r){let i=h(),a=k[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`,img:`ch1.png`},o=A(i,e.id,`vocabulary`,t.length),s=A(i,e.id,`exercises`,n.length);return`
  ...
  `}function M(e){document.getElementById(`btn-back`)?.addEventListener(`click`,()=>{D(`/`)}),document.querySelectorAll(`.section-btn`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.section;if(n===`grammar`){alert(`Грамматика — скоро!`);return}D(`/chapter/${e}/${n}`)})})}var N={ch1:{color:`#7C3AED`,bg:`#EDE7FB`},ch2:{color:`#EC4899`,bg:`#FAE9F2`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`},ch4:{color:`#10B981`,bg:`#E8F7EE`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`}},P=0,F=[],I=``,L=!1;function R(e,t){I=e.id,F=t,P=0,L=!1;let n=N[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`};return`
    <div class="vocab-page">
      <header class="header">
        <button class="btn-back" id="btn-back-vocab">← ${e.title_fi}</button>
        <span class="vocab-counter" id="vocab-counter">1 / ${F.length}</span>
      </header>
      ...
      <div class="vocab-container">
        <div class="flashcard" id="flashcard" style="--card-color: ${n.color}; --card-bg: ${n.bg}">
          <div class="flashcard-inner" id="flashcard-inner">
            <div class="flashcard-front">
              ${F[0]?.tag===`puhekieli`?`<span class="puhekieli-badge">🗣️ puhekieli</span>`:``}
              <span class="card-lang">финский</span>
              <div class="card-word" id="card-word-front">${F[0]?.finnish||``}</div>
              <span class="card-hint">нажми чтобы перевернуть</span>
            </div>
            <div class="flashcard-back">
              ${F[0]?.tag===`puhekieli`?`<span class="puhekieli-badge">🗣️ puhekieli</span>`:``}
              <span class="card-lang">перевод</span>
              <div class="card-word" id="card-word-back">${F[0]?.russian||``}</div>
              ${F[0]?.english?`<div class="card-word-en">${F[0].english}</div>`:``}
            </div>
          </div>
        </div>
        ...
        <div class="vocab-dots" id="vocab-dots">
          ${F.map((e,t)=>`<span class="vocab-dot ${t===0?`active`:``}" data-i="${t}"></span>`).join(``)}
        </div>
      </div>
    </div>
  `}function z(e){let t=N[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`};I=e.id,document.getElementById(`btn-back-vocab`)?.addEventListener(`click`,()=>{D(`/chapter/${e.id}`)});let n=document.getElementById(`flashcard-inner`);document.getElementById(`flashcard`)?.addEventListener(`click`,()=>{L||(L=!0,n.classList.add(`flipped`),document.getElementById(`vocab-actions`).style.display=`flex`)});function r(r){if(v(p().activeProfile,I,P,r),P++,P>=F.length){ee(e,t);return}L=!1,n.classList.remove(`flipped`),document.getElementById(`vocab-actions`).style.display=`none`;let i=F[P];document.getElementById(`card-word-front`).textContent=i.finnish,document.getElementById(`card-word-back`).textContent=i.russian,document.querySelectorAll(`.flashcard-front, .flashcard-back`).forEach(e=>{let t=e.querySelector(`.puhekieli-badge`);if(i.tag===`puhekieli`){if(!t){let t=document.createElement(`span`);t.className=`puhekieli-badge`,t.textContent=`🗣️ puhekieli`,e.prepend(t)}}else t?.remove()}),document.getElementById(`vocab-counter`).textContent=`${P+1} / ${F.length}`,document.querySelectorAll(`.vocab-dot`).forEach((e,t)=>{e.classList.toggle(`active`,t===P),t<P&&e.classList.add(`done`)})}document.getElementById(`btn-hard`)?.addEventListener(`click`,()=>r(!1)),document.getElementById(`btn-ok`)?.addEventListener(`click`,()=>r(!1)),document.getElementById(`btn-know`)?.addEventListener(`click`,()=>r(!0))}function ee(e,t){document.querySelector(`.vocab-container`).innerHTML=`
    <div class="complete-screen">
      <div class="complete-emoji">🎉</div>
      <h2>Словарь главы пройден!</h2>
      <p style="color: #6B7280; margin-top: 8px">${F.length} слов изучено</p>
      <button class="btn-complete" style="background: ${t.color}" id="btn-to-chapter">
        Вернуться к главе →
      </button>
    </div>
  `,document.getElementById(`btn-to-chapter`)?.addEventListener(`click`,()=>{D(`/chapter/${e.id}`)})}var te={ch1:{color:`#7C3AED`,bg:`#EDE7FB`},ch2:{color:`#EC4899`,bg:`#FAE9F2`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`},ch4:{color:`#10B981`,bg:`#E8F7EE`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`}},ne={multiple_choice:`☑️ Выбери правильный`,fill_blank:`✏️ Заполни пропуск`,self_check:`👁️ Самопроверка`,matching:`🔗 Соедини пары`,vowel_harmony:`🔤 Гармония гласных`},B=[],V=0,H=null,U=null,W=!1;function re(e,t){return H=e,B=t,V=0,W=!1,U=te[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`},B.length?`
    <div class="ex-page">
      <header class="header">
      ...
      <div class="ex-progress-bar">
        <div class="ex-progress-fill" id="ex-progress-fill"
             style="width: ${ie(0)}%; background: ${U.color}"></div>
      </div>
  ...
  `:`
      ...
      </div>`}function ie(e){return Math.round(e/B.length*100)}function ae(e){return e.map(e=>({v:e,sort:Math.random()})).sort((e,t)=>e.sort-t.sort).map(({v:e})=>e)}function G(e,t){let n=e.type||`multiple_choice`,r=ne[n]||n,i=[e.option_a,e.option_b,e.option_c,e.option_d].filter(Boolean),a=`abcd`.indexOf((e.correct||`a`).toLowerCase()),o=i[a]||i[0],s=n===`self_check`?i:ae(i.map((e,t)=>({text:e,isCorrect:t===a})));return n===`self_check`?`
  ...
  `}function oe(e){H=e,U=te[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`},document.getElementById(`btn-back-ex`)?.addEventListener(`click`,()=>{D(`/chapter/${e.id}`)}),K()}function K(){let e=B[V];e&&(W=!1,document.getElementById(`btn-show-answer`)?.addEventListener(`click`,()=>{document.getElementById(`answer-reveal`).style.display=`block`,document.getElementById(`btn-show-answer`).style.display=`none`}),document.querySelectorAll(`.self-btn`).forEach(e=>{e.addEventListener(`click`,()=>{e.dataset.result,_(p().activeProfile,H.id,V,e.dataset.result),q()})}),document.querySelectorAll(`.ex-option`).forEach(t=>{t.addEventListener(`click`,()=>{if(W)return;W=!0;let n=t.dataset.correct===`true`,r=document.querySelector(`.ex-card`).dataset.correct;document.querySelectorAll(`.ex-option`).forEach(e=>{e.dataset.correct===`true`?e.classList.add(`correct`):e===t&&!n&&e.classList.add(`wrong`),e.disabled=!0});let i=document.getElementById(`ex-feedback`);i.style.display=`block`,i.innerHTML=n?`<div class="feedback-correct" style="border-color: ${U.color}">
           ...
           <button class="ex-next-btn" id="btn-next" style="background: ${U.color}">
             ${V+1<B.length?`Следующее →`:`Завершить 🎉`}
           </button>`,_(p().activeProfile,H.id,V,n?`right`:`wrong`),document.getElementById(`btn-next`)?.addEventListener(`click`,q)})}))}function q(){if(V++,V>=B.length){se();return}let e=document.getElementById(`ex-container`);e.innerHTML=G(B[V],V),document.getElementById(`ex-counter`).textContent=`${V+1} / ${B.length}`,document.getElementById(`ex-progress-fill`).style.width=`${ie(V)}%`,K()}function se(){let e=document.getElementById(`ex-container`);e.innerHTML=`
    <div class="complete-screen">
      <div class="complete-emoji">🏆</div>
  ...
  `,document.getElementById(`btn-to-chapter-ex`)?.addEventListener(`click`,()=>{D(`/chapter/${H.id}`)})}function ce(e){let t=p().profiles[e],{totalWords:n,totalExercises:r,totalChapters:i,streak:a,activity:o}=y(e);return`
  ...
  `}function ge(e,t){let n=h();return`
    ${he(e)}
    <div class="hero">
      <div class="hero-text">
    ...
    ${_e(t,n)}
  `}function _e(e,t){let n={};return e.forEach((e,t)=>{let r=e.level||`A0–A1`;n[r]||(n[r]=[]),n[r].push({...e,globalIndex:t})}),fe.filter(e=>n[e]).map(e=>{let r=pe[e]||{color:`#7C3AED`,bg:`#EDE7FB`},i=n[e],a=i.reduce((e,n)=>e+Y(t,n.id),0)/i.length;return`
        <div class="level-section">
          ...
          <div class="chapters-grid">
            ${i.map((e,n)=>ve(e,e.globalIndex+1,t)).join(``)}
          </div>
        </div>
      `}).join(``)}function ve(e,t,n){let r=Y(n,e.id),i=r===100,a=r===0?`Начать`:i?`Повторить`:`Продолжить →`;return`
  ...
  `}function X(e){document.querySelector(`#app`).innerHTML=e}function ye(){document.querySelectorAll(`.profile-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.profile;p().activeProfile===t?D(`/profile/${t}`):(g(t),Q())})}),document.querySelectorAll(`.chapter-card`).forEach(e=>{e.addEventListener(`click`,()=>{D(`/chapter/${e.dataset.chapter}`)})})}var Z=de.map(e=>({...e,...J[e.id]}));function Q(){X(ge(p(),Z)),ye()}var $={chapters:[],vocabulary:[],exercises:[]};async function be(){document.querySelector(`#app`).innerHTML=`<div class="loading">Загружаем финский... 🇫🇮</div>`;try{$=await T(),$.chapters?.length>0&&(Z=$.chapters.map(e=>({...e,...J[e.id]})))}catch{console.log(`Sheets недоступен, используем demo данные`)}xe(),O(xe)}function xe(){let e=E(),t=p();if(e.page===`dashboard`||!e.page){Q();return}if(e.page===`profile`){X(ce(e.profileId)),ue();return}let n=Z.find(t=>t.id===e.chapterId);if(!n){Q();return}let r=$.vocabulary.filter(t=>t.chapter_id===e.chapterId),i=$.exercises.filter(t=>t.chapter_id===e.chapterId);if(e.section===`vocabulary`){X(R(n,r)),z(n);return}if(e.section===`exercises`){X(re(n,i)),oe(n);return}X(j(n,r,i,t)),M(e.chapterId)}be();