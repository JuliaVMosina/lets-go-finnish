...
`,`"`,s.BYTE_ORDER_MARK],s.WORKERS_SUPPORTED=!r&&!!t.Worker,s.NODE_STREAM_INPUT=1,s.LocalChunkSize=10485760,s.RemoteChunkSize=5242880,s.DefaultDelimiter=`,`,s.Parser=h,s.ParserHandle=p,s.NetworkStreamer=l,s.FileStreamer=u,s.StringStreamer=d,s.ReadableStreamStreamer=f,t.jQuery&&((n=t.jQuery).fn.parse=function(e){var r=e.config||{},i=[];return this.each(function(e){if(!(n(this).prop(`tagName`).toUpperCase()===`INPUT`&&n(this).attr(`type`).toLowerCase()===`file`&&t.FileReader)||!this.files||this.files.length===0)return!0;for(var a=0;a<this.files.length;a++)i.push({file:this.files[a],inputElem:this,instanceConfig:n.extend({},r)})}),a(),this;function a(){if(i.length===0)x(e.complete)&&e.complete();else{var t,r,a,c,l=i[0];if(x(e.before)){var u=e.before(l.file,l.inputElem);if(typeof u==`object`){if(u.action===`abort`)return t=`AbortError`,r=l.file,a=l.inputElem,c=u.reason,void(x(e.error)&&e.error({name:t},r,a,c));if(u.action===`skip`)return void o();typeof u.config==`object`&&(l.instanceConfig=n.extend(l.instanceConfig,u.config))}else if(u===`skip`)return void o()}var d=l.instanceConfig.complete;l.instanceConfig.complete=function(e){x(d)&&d(e,l.file,l.inputElem),o()},s.parse(l.file,l.instanceConfig)}}function o(){i.splice(0,1),a()}}),i&&(t.onmessage=function(e){e=e.data,s.WORKER_ID===void 0&&e&&(s.WORKER_ID=e.workerId),typeof e.input==`string`?t.postMessage({workerId:s.WORKER_ID,results:s.parse(e.input,e.config),finished:!0}):(t.File&&e.input instanceof File||e.input instanceof Object)&&(e=s.parse(e.input,e.config))&&t.postMessage({workerId:s.WORKER_ID,results:e,finished:!0})}),(l.prototype=Object.create(c.prototype)).constructor=l,(u.prototype=Object.create(c.prototype)).constructor=u,(d.prototype=Object.create(d.prototype)).constructor=d,(f.prototype=Object.create(c.prototype)).constructor=f,s})}))(),1),_=`2PACX-1vQwqkX44b-HPNysrWI2Qy5AtbUv9DVW8JtDXiFBvoe7lkIbkp-TNP2VjLyli2AUzFFCHZH0HYvcfytH`,v={chapters:`0`,vocabulary:`195278083`,exercises:`1198814770`};function y(e){return`https://docs.google.com/spreadsheets/d/e/${_}/pub?gid=${e}&single=true&output=csv`}async function b(e){let t=await(await fetch(y(e))).text();return g.default.parse(t,{header:!0,skipEmptyLines:!0}).data}async function x(){let[e,t,n]=await Promise.all([b(v.chapters),b(v.vocabulary),b(v.exercises)]);return{chapters:e,vocabulary:t,exercises:n}}function S(){let e=(location.hash.replace(`#`,``)||`/`).split(`/`).filter(Boolean);return e.length===0?{page:`dashboard`}:e[0]===`chapter`&&e[1]?{page:e[2]||`chapter`,chapterId:e[1],section:e[2]||null}:{page:`dashboard`}}function C(e){location.hash=e}function w(e){window.addEventListener(`hashchange`,e)}var T={ch1:{color:`#7C3AED`,bg:`#EDE7FB`,img:`ch1.png`},ch2:{color:`#EC4899`,bg:`#FAE9F2`,img:`ch2.png`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`,img:`ch3.png`},ch4:{color:`#10B981`,bg:`#E8F7EE`,img:`ch4.png`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`,img:`ch5.png`}};function E(e,t,n,r){let i=e[n]?.[t]||{},a=Object.values(i).filter(e=>e.completed||e.seen).length;return r>0?Math.round(a/r*100):0}function D(e,t,n,r){let i=p(),a=T[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`,img:`ch1.png`},o=E(i,e.id,`vocabulary`,t.length),s=E(i,e.id,`exercises`,n.length);return`
  ...
  `}function O(e){document.getElementById(`btn-back`)?.addEventListener(`click`,()=>{C(`/`)}),document.querySelectorAll(`.section-btn`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.section;if(n===`grammar`){alert(`Грамматика — скоро!`);return}C(`/chapter/${e}/${n}`)})})}var k={ch1:{color:`#7C3AED`,bg:`#EDE7FB`},ch2:{color:`#EC4899`,bg:`#FAE9F2`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`},ch4:{color:`#10B981`,bg:`#E8F7EE`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`}},A=0,j=[],M=``,N=!1;function P(e,t){M=e.id,j=t,A=0,N=!1;let n=k[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`};return`
    <div class="vocab-page">
      <header class="header">
        <button class="btn-back" id="btn-back-vocab">← ${e.title_fi}</button>
        <span class="vocab-counter" id="vocab-counter">1 / ${j.length}</span>
      </header>
      ...
      <div class="vocab-container">
        <div class="flashcard" id="flashcard" style="--card-color: ${n.color}; --card-bg: ${n.bg}">
          <div class="flashcard-inner" id="flashcard-inner">
            <div class="flashcard-front">
              ...
              <div class="card-word" id="card-word-front">${j[0]?.finnish||``}</div>
              <span class="card-hint">нажми чтобы перевернуть</span>
            </div>
            <div class="flashcard-back">
              <span class="card-lang">перевод</span>
              <div class="card-word" id="card-word-back">${j[0]?.russian||``}</div>
              ${j[0]?.english?`<div class="card-word-en">${j[0].english}</div>`:``}
            </div>
          </div>
        </div>
        ...
        <div class="vocab-dots" id="vocab-dots">
          ${j.map((e,t)=>`<span class="vocab-dot ${t===0?`active`:``}" data-i="${t}"></span>`).join(``)}
        </div>
      </div>
    </div>
  `}function F(e){let t=k[e.id]||{color:`#7C3AED`,bg:`#EDE7FB`};M=e.id,document.getElementById(`btn-back-vocab`)?.addEventListener(`click`,()=>{C(`/chapter/${e.id}`)});let n=document.getElementById(`flashcard-inner`);document.getElementById(`flashcard`)?.addEventListener(`click`,()=>{N||(N=!0,n.classList.add(`flipped`),document.getElementById(`vocab-actions`).style.display=`flex`)});function r(r){if(h(d().activeProfile,M,A,r),A++,A>=j.length){I(e,t);return}N=!1,n.classList.remove(`flipped`),document.getElementById(`vocab-actions`).style.display=`none`,document.getElementById(`card-word-front`).textContent=j[A].finnish,document.getElementById(`card-word-back`).textContent=j[A].russian,document.getElementById(`vocab-counter`).textContent=`${A+1} / ${j.length}`,document.querySelectorAll(`.vocab-dot`).forEach((e,t)=>{e.classList.toggle(`active`,t===A),t<A&&e.classList.add(`done`)})}document.getElementById(`btn-hard`)?.addEventListener(`click`,()=>r(!1)),document.getElementById(`btn-ok`)?.addEventListener(`click`,()=>r(!1)),document.getElementById(`btn-know`)?.addEventListener(`click`,()=>r(!0))}function I(e,t){document.querySelector(`.vocab-container`).innerHTML=`
    <div class="complete-screen">
      <div class="complete-emoji">🎉</div>
      <h2>Словарь главы пройден!</h2>
      <p style="color: #6B7280; margin-top: 8px">${j.length} слов изучено</p>
      <button class="btn-complete" style="background: ${t.color}" id="btn-to-chapter">
        Вернуться к главе →
      </button>
    </div>
  `,document.getElementById(`btn-to-chapter`)?.addEventListener(`click`,()=>{C(`/chapter/${e.id}`)})}var L={ch1:{color:`#7C3AED`,bg:`#EDE7FB`,img:`ch1.png`},ch2:{color:`#EC4899`,bg:`#FAE9F2`,img:`ch2.png`},ch3:{color:`#F59E0B`,bg:`#FEEEDB`,img:`ch3.png`},ch4:{color:`#10B981`,bg:`#E8F7EE`,img:`ch4.png`},ch5:{color:`#3B82F6`,bg:`#E9EEFB`,img:`ch5.png`}},R=[{id:`ch1`,title_fi:`Hei ja tervetuloa`,title_ru:`Привет и добро пожаловать`,level:`A0–A1`},{id:`ch2`,title_fi:`Minkämaalainen sinä olet?`,title_ru:`Ты откуда?`,level:`A0–A1`},{id:`ch3`,title_fi:`Pedro soittaa Mikkolle`,title_ru:`Педро звонит Микко`,level:`A0–A1`},{id:`ch4`,title_fi:`Minä`,title_ru:`Я`,level:`A0–A1`},{id:`ch5`,title_fi:`Meidän tavallinen päivä`,title_ru:`Наш обычный день`,level:`A0–A1`}],z=[`A0–A1`,`A1–A2`,`A2–B1`,`B1–B2`],B={"A0–A1":{color:`#7C3AED`,bg:`#EDE7FB`},"A1–A2":{color:`#EC4899`,bg:`#FAE9F2`},"A2–B1":{color:`#F59E0B`,bg:`#FEEEDB`},"B1–B2":{color:`#10B981`,bg:`#E8F7EE`}};function V(e){let t=new Date().getHours();return t>=6&&t<12?`Hyvää huomenta, ${e}! ☀️ Готова к финскому?`:t>=12&&t<18?`Hei hei, ${e}! Продолжим?`:`Hyvää iltaa, ${e}! Один урок перед сном?`}function H(e,t,n=20){let r=e.exercises[t]||{},i=e.vocabulary[t]||{},a=Object.values(r).filter(e=>e.completed).length+Object.values(i).filter(e=>e.seen).length;return Math.min(100,Math.round(a/n*100))}function U(e){let t=e.profiles;return`
    <header class="header">
      <div class="logo">
  ...
  `}function W(e,t){let n=p();return`
    ...
    ${G(t,n)}
  `}function G(e,t){let n={};return e.forEach((e,t)=>{let r=e.level||`A0–A1`;n[r]||(n[r]=[]),n[r].push({...e,globalIndex:t})}),z.filter(e=>n[e]).map(e=>{let r=B[e]||{color:`#7C3AED`,bg:`#EDE7FB`},i=n[e],a=i.reduce((e,n)=>e+H(t,n.id),0)/i.length;return`
        <div class="level-section">
          ...
          <div class="chapters-grid">
            ${i.map((e,n)=>K(e,e.globalIndex+1,t)).join(``)}
          </div>
        </div>
      `}).join(``)}function K(e,t,n){let r=H(n,e.id),i=r===100,a=r===0?`Начать`:i?`Повторить`:`Продолжить →`;return`
  ...
  `}function q(e){document.querySelector(`#app`).innerHTML=e}function J(){document.querySelectorAll(`.profile-btn`).forEach(e=>{e.addEventListener(`click`,()=>{m(e.dataset.profile),X()})}),document.querySelectorAll(`.chapter-card`).forEach(e=>{e.addEventListener(`click`,()=>{C(`/chapter/${e.dataset.chapter}`)})})}var Y=R.map(e=>({...e,...L[e.id]}));function X(){q(W(d(),Y)),J()}var Z={chapters:[],vocabulary:[],exercises:[]};async function Q(){document.querySelector(`#app`).innerHTML=`<div class="loading">Загружаем финский... 🇫🇮</div>`;try{Z=await x(),Z.chapters?.length>0&&(Y=Z.chapters.map(e=>({...e,...L[e.id]})))}catch{console.log(`Sheets недоступен, используем demo данные`)}$(),w($)}function $(){let e=S(),t=d();if(e.page===`dashboard`||!e.page){X();return}let n=Y.find(t=>t.id===e.chapterId);if(!n){X();return}let r=Z.vocabulary.filter(t=>t.chapter_id===e.chapterId),i=Z.exercises.filter(t=>t.chapter_id===e.chapterId);if(e.section===`vocabulary`){q(P(n,r)),F(n);return}if(e.section===`exercises`){q(`<div class="loading">Упражнения — скоро! 💪</div>`);return}q(D(n,r,i,t)),O(e.chapterId)}Q();